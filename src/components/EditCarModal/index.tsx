...
const EditCarModal = ({ editIsOpen, setEditIsOpen, carData }: iEditCarModal) => {
  ...
  const [car, setCar] = useState<iCar | null>(carData);
  ...
  useEffect(() => {
    const getCarsData = async () => {
      try {
        ...
        let newImageCount = [];
        const carImages = String(carData?.images.gallery).replace(/[}"\{]/g, "").split(",");
          ...
          <ModalBody>
            ...
            >
              ...
              >
                {brands ? (
                  brands.map((brand: string) => (
                    <option key={brand} value={brand} selected={brand==carData?.brand}>
                      {brand}
                    </option>
                  ))
                ) : (
              ...
              >
                {cars ? (
                  cars.map((car: iCar) => (
                    <option key={car.id} value={car.name} selected={car.name==carData?.model}>
                      {car.name}
                    </option>
                  ))
                ) : (
              ...
              <Flex gap={"11px"}>
                <InputForm
                  value={Number(car?.value? car.value : car?.price)}
                  readOnly={true}
                  id="7"
                  type="text"
              ...
              <InputForm
                ...
                error={errors.cover}
              />
              {String(carData?.images.gallery).replace(/[}"\{]/g, "").split(",").map((imageUrl: string, i, array) => {
                return (
                <InputForm
                  id={`${11 + i}`}
                  key={`${11 + i}`}
              ...
              {
                imgCount.length + newImgCount.length <= 6 && newImgCount.map( (i) => (
                  <InputForm
              ...
              <Button
                ...
                size={"big"}
                p={"10px"}
                disabled={imgCount.length + newImgCount.length === 6? true : false}
                onClick={() => {
