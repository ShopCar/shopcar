import { Button, Flex, FormLabel, Heading, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Select, useDisclosure } from "@chakra-ui/react"
...
const RegisterCarModal = ({isOpen, setIsOpen}: iRegisterCarModal) => {
  ...
  const formSchame = z.object({
    ...
    gallery: z.array(
        z.string().min(5, "imagem é obrigatória")
    ).max(6)
  });
      ...
      <Modal isOpen={isOpen} onClose={() => {setIsOpen(false)}}>
        ...
        <ModalContent>
          ...
          >
            ...
            <FormLabel htmlFor="brandsSelect">
              Marca
            </FormLabel>
            <Select {...register("brand")} id="brandsSelect" onChange={async (e) => {
            ...
            <FormLabel htmlFor="modelsSelect">
              Modelo
            </FormLabel>
            <Select {...register("model")} id="modelsSelect" onChange={async (e) => {
                const currentCar = cars!.filter((elem) => elem.name === e.target.value)[0]
                setCar(currentCar)
              }}>
              {cars? (
                cars.map((car: iCar) => (
            ...
            <Flex gap={"11px"}>
              ...
              />
              <InputForm
                value={car?.fuel == 1? "Flex" : (car?.fuel == 2? "Híbrido" : (car?.fuel == 3? "Elétrico" : "Combustível"))}
                readOnly={true}
                id="4"
                type="text"
                label="Combustível"
            ...
            <Button variant={"brandOpacity"} size={"big"} p={"10px"} onClick={() => {
          ...
          <ModalFooter>
            <Button variant='negative' size={"big"} p={"10px"} mr={3} onClick={() => {setIsOpen(false)}}>
              Cancelar
            </Button>
            <Button type="submit" onClick={handleSubmit(onSubmit)} variant='brand1' size={"big"} p={"10px"}>Criar anúncio</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
    </>
